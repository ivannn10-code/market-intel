...
ИСТОЧНИКИ (важно — это публичный пост в TG-канал):
- Источник встраиваешь В ПРОЗУ, без @-ссылок на чужие каналы: «по данным ЦБ РФ», «по информации Росреестра / ЕИСЖС / ДОМ.РФ», «по информации MR Group / STONE / ПИК» (это сами девелоперы, можно), «по моему опыту», «по моим расчётам», «я думаю / моё мнение».
- НИКОГДА не упоминать @-ник медиа/новостных каналов (@nedvizha, @novostroyman, @stone_developer и т.п.) — мы не продвигаем чужие каналы.
- НЕТ блока «📨 Источники: …» в конце поста. НЕТ скобок типа «(@channel, дата)». НЕТ ссылок-URL.

ОБЯЗАТЕЛЬНО:
- Расшифровывать NOI (чистый операционный доход), IRR (внутренняя норма доходности), Cap rate, ДКПБН (договор купли-продажи будущей недвижимости), ДДУ, УК (управляющая компания), ПВ (первоначальный взнос), CPI (индекс потребительских цен) при первом упоминании.
- НЕ выдумывать цифры. Нет в фактуре — не упоминать. Если оценка — пометь словом «по моим прикидкам» / «по моим расчётам».
- НЕ голословно критиковать ЖК / застройщиков. Любой минус — только на верифицируемых данных.

ФОРМАТ ВЫВОДА (это ГОТОВЫЙ к публикации пост в TG-канал, без редактуры):
- НЕ ставь meta-заголовок типа «# Пост-разбор · Telegram · Сегмент N · #рубрика». Это служебная инфа, в канал она НЕ идёт.
- НЕ используй markdown-разделители «---» между абзацами. Используй ПУСТУЮ строку.
- НЕ ставь заголовки H1/H2 (`#`, `##`) — TG их не рендерит.
- Допустимы: **жирный** для акцентов, обычные эмодзи (умеренно), маркированный список через «—» или «•», курсив `_слово_`.
- Текст НАЧИНАЕТСЯ С ХУКА. Никаких «Сегодня поговорим», «В этом посте разберём».
- Никаких @-ников в выходе.
...
def main() -> int:
    ...
    # Программная санитизация (последний рубеж — убирает meta/---/Источники:/@)
    if agent_team is not None:
        try:
            body = agent_team.sanitize_post(body)
        except Exception as _exc:
            print(f"[gen] sanitize warn: {_exc}")
    ...
    # Авто-обложка в hero-стиле для публикуемых рубрик (только если --image-path не задан)
    image_rel = args.image_path
    COVER_RUBRICS = {"review", "number", "compare", "method", "digest"}
    if not image_rel and rubric in COVER_RUBRICS and not args.no_team:
        try:
            print(f"[gen] → автогенерация обложки (Claude + Gemini)...")
            sys.path.insert(0, str(HERE / "visual"))
            import carousel_builder as cb
            client = anthropic.Anthropic(api_key=os.environ["ANTHROPIC_API_KEY"])
            content = cb.generate_cover_content(body[:3000], client, MODEL)
            if content:
                year, week = iso_week(date)
                weekday = WEEKDAY_CODES[date.weekday()]
                cover_dir = HERE / "covers" / f"{year}-W{week:02d}" / f"{date.isoformat()}-{weekday}-{rubric}"
                png = cb.build_cover(content, cover_dir, ai_bg=True)
                if png and png.exists():
                    image_rel = str(png.relative_to(HERE))
                    print(f"[gen] ✓ обложка: {image_rel}")
        except Exception as exc:
            print(f"[gen] cover skip: {exc}")

    path = save_post(date, rubric, body, image_path=image_rel, sources=args.source or [],
