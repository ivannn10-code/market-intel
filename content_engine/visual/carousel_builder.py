...
:root{--bg:#0F3D4A;--text:#E7E2D8;--accent:#FF5A2A;--steel:#4A6A7B;--muted:#9DB1BC;--watermark:rgba(231,226,216,0.55);}
...
/* fallback-фон, если AI-картинки нет — тёмная бирюза с мягким оранжевым свечением */
.slide.no-bg{background:
  radial-gradient(120% 90% at 78% 18%, rgba(255,90,42,0.30) 0%, rgba(255,90,42,0) 55%),
  radial-gradient(90% 70% at 12% 88%, rgba(74,106,123,0.30) 0%, rgba(74,106,123,0) 60%),
  linear-gradient(160deg, #14495A 0%, #0F3D4A 60%, #0A2C36 100%);}
.slide::before{content:'';position:absolute;inset:0;z-index:1;}
.slide.overlay-bottom::before{background:linear-gradient(180deg,
  rgba(11,46,56,0.40) 0%,rgba(11,46,56,0.15) 24%,rgba(11,46,56,0.22) 50%,
  rgba(11,46,56,0.86) 86%,rgba(11,46,56,0.96) 100%);}
.slide.overlay-strong::before{background:linear-gradient(180deg,
  rgba(11,46,56,0.93) 0%,rgba(11,46,56,0.74) 38%,rgba(11,46,56,0.62) 68%,rgba(11,46,56,0.92) 100%);}
/* CTA-финал: бирюза-доминанта + оранжевое свечение (не сплошная заливка) */
.slide.overlay-cta::before{background:
  radial-gradient(95% 80% at 84% 14%, rgba(255,90,42,0.40) 0%, rgba(255,90,42,0) 56%),
  linear-gradient(180deg, rgba(11,46,56,0.90) 0%,rgba(11,46,56,0.70) 46%,rgba(11,46,56,0.93) 100%);}
...
.pager-pill{font-family:'PT Serif',serif;font-weight:700;font-size:24px;letter-spacing:0.8px;color:var(--text);
  background:rgba(11,46,56,0.70);border:1px solid rgba(231,226,216,0.25);border-radius:100px;
...
/* HERO */
.hero-quote-section{margin-top:150px;max-width:600px;margin-left:auto;margin-right:0;text-align:right;}
.hero-quote{font-family:'PT Serif',serif;font-style:italic;font-size:50px;line-height:1.28;color:var(--accent);}
/* Варианты читаемости hero-цитаты на любом фоне */
.hero-quote.card-a{display:inline-block;background:rgba(11,46,56,0.78);
  -webkit-backdrop-filter:blur(12px);backdrop-filter:blur(12px);
  border-radius:16px;padding:22px 28px;box-shadow:0 10px 36px rgba(0,0,0,0.30);text-align:left;}
.hero-quote.bold-light{font-style:normal;font-weight:700;color:var(--text);letter-spacing:-0.5px;
  text-shadow:0 3px 18px rgba(0,0,0,0.88),0 0 14px rgba(0,0,0,0.55);}
.hero-quote.card-bar{display:inline-block;background:rgba(11,46,56,0.82);
  -webkit-backdrop-filter:blur(12px);backdrop-filter:blur(12px);
  border-left:4px solid var(--accent);border-radius:4px 14px 14px 4px;
  padding:22px 28px 22px 26px;box-shadow:0 10px 36px rgba(0,0,0,0.30);
  font-style:normal;font-weight:700;color:var(--text);text-align:left;}
...
.hero-headline em{font-style:italic;font-weight:400;color:rgba(231,226,216,0.78);}
...
.hero-stat .l{font-size:20px;letter-spacing:1.5px;text-transform:uppercase;color:rgba(231,226,216,0.7);}
...
.calc-row{display:flex;align-items:baseline;justify-content:space-between;gap:28px;padding:22px 0;
  border-bottom:1px solid rgba(231,226,216,0.16);}
.calc-row:last-child{border-bottom:none;}
.calc-label{font-size:27px;line-height:1.32;color:var(--text);flex:1;}
.calc-label .muted{display:block;color:rgba(231,226,216,0.55);font-size:18px;margin-top:6px;}
...
.calc-final-label{font-weight:600;font-size:17px;letter-spacing:2.4px;text-transform:uppercase;
  color:rgba(231,226,216,0.72);margin-bottom:12px;}
...
.calc-final-formula{font-size:20px;color:rgba(231,226,216,0.65);margin-top:14px;line-height:1.4;}
...
.compare-divider{width:1px;background:rgba(255,90,42,0.55);}
...
.compare-formula{font-size:21px;line-height:1.4;color:rgba(231,226,216,0.72);}
.compare-summary{margin-top:40px;padding-top:24px;border-top:1px solid rgba(231,226,216,0.18);
...
/* footnote-источники */
.content-footer{font-size:16px;line-height:1.4;color:rgba(231,226,216,0.55);max-width:80%;margin-top:34px;}
/* CTA */
...
.cta-handle{font-weight:600;font-size:22px;letter-spacing:0.8px;color:var(--text);
  background:rgba(11,46,56,0.55);border:1px solid rgba(231,226,216,0.35);border-radius:100px;padding:10px 22px;}
"""
...
def slide_hero(idx: int, total: int, h: dict, bg_file: str | None) -> str:
    quote = ""
    if h.get("quote"):
        klass = _QUOTE_STYLES.get((h.get("quote_style") or "a").lower(), "card-a")
        quote = f'<div class="hero-quote-section"><div class="hero-quote {klass}">{fmt(h["quote"])}</div></div>'
    stat = ""
    st = h.get("stat") or {}
    if st.get("value"):
...
_QUOTE_STYLES = {"a": "card-a", "b": "bold-light", "c": "card-bar"}


def slide_cover(h: dict, bg_file: str | None, quote_style: str = "a") -> str:
    """Самостоятельная обложка из текста — hero-стиль, БЕЗ крупной цифры (не слайд карусели).
    quote_style: 'a' (card backdrop) | 'b' (light bold + shadow) | 'c' (card + orange bar)."""
    quote = ""
    if h.get("quote"):
        klass = _QUOTE_STYLES.get(quote_style.lower(), "card-a")
        quote = f'<div class="hero-quote-section"><div class="hero-quote {klass}">{fmt(h["quote"])}</div></div>'
    stat = ""
    st = h.get("stat") or {}
    if st.get("value"):
...
def slide_cta(idx: int, total: int, cta: dict, bg_file: str | None) -> str:
    ...
    inner = (
        ...
        + '<div class="cta-author-meta">Недвижимость Москвы · 12 лет на рынке</div></div></div>'
        + '</div>'
    )
        ...
        "properties": {
            "hero": {
                ...
                "properties": {
                    ...
                    "quote_style": {"type": "string", "enum": ["a", "c"],
                                    "description": "Стиль hero-цитаты: 'a' — журнальный эпиграф (по умолчанию, для аналитики/методологии/расчёта/лотов); 'c' — постерный pull-quote (для red flags / предупреждений / критических ошибок / срочного)"},
                },
...
hero.quote_style — выбери осознанно под тон темы карусели:
- 'a' (по умолчанию, ~80%): аналитика, разбор, методология, расчёт, цифра дня, лот, кейс — журнальный эпиграф.
- 'c' (исключение): red flags, предупреждения, критические ошибки, срочное, тревожные сигналы — постерный pull-quote с весом.
...
COVER_TOOL = {
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "quote_style": {"type": "string", "enum": ["a", "c"],
                            "description": "Стиль hero-цитаты: 'a' — журнальный эпиграф (оранжевый italic, мягкий, по умолчанию); 'c' — постерный pull-quote (светлая bold + оранжевая отбивка, для red flags / предупреждений / критических ошибок / срочного)"},
        },
        "required": ["label", "title", "bg_hint"],
    },
}
...
quote_style — выбери осознанно под тон темы:
- 'a' (по умолчанию, ~80% контента): аналитика, разбор, методология, расчёт, цифра дня, лот, кейс. Цитата = мягкий журнальный эпиграф.
- 'c' (исключение): red flags, предупреждения, критические ошибки, срочное, «опасно сейчас», тревожные сигналы рынка. Цитата = постерный pull-quote с весом."""
...
def build_cover(content: dict, out_dir: Path, ai_bg: bool = True, progress=None, quote_style: str | None = None) -> Path:
    # Если quote_style не задан явно — берём из контента Claude (он выбирает 'a' или 'c' под тон).
    if quote_style is None:
        quote_style = (content.get("quote_style") or "a")
    ...
    html = slide_cover(content, bg_file, quote_style=quote_style)
