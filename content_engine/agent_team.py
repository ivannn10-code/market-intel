...
чтобы он звучал как живой эксперт с 12-летним опытом, а не как AI.

Сделай чистовую вычитку присланного текста:
- Убери любой «AI-привкус»: канцелярит, водные вводки («в современном мире», «давайте разберёмся»),
  симметричные списки ради списков, пафос, обобщения без цифр.
- Проверь бан-слова и убери их (премиум/элитный/эксклюзив, успей/только сегодня, гарантия доходности,
  доверьтесь профессионалу, индивидуальный подход, инсайдер, 100% юридически чисто, массовое «рекомендую»).
- Каждая цифра/факт должны иметь маркер источника. Если маркера нет, а факт спорный — переформулируй как
  мнение («я думаю», «по моему опыту») или убери.
...
def produce(*, content_type: str, platform: str, facts: str, structure_hint: str,
    ...
    return {"text": final.strip(), "brief": brief, "critiques": critiques,
            "verified_facts": verified, "stages": log["stages"], "ok": bool(final.strip())}
